...
public class BoardController {
	...
	@ModelAttribute("condMap") //model.addAttribute("condMap(키)" , conditionMap(값));
	public Map<String, String> searchConditionMap(){
		...
		public String boardUpdate(@ModelAttribute("board") BoardVO vo
				,SessionStatus st) {
			//세션이 같은 이름에 board가 있으면 세션에 저장된 board를 파라미터로 사용
			System.out.println("=====================update 파라미터 확인 \n"+vo);
			System.out.println(vo);
			service.updateBoard(vo);
			st.setComplete();//session cleanup
	...
	public String boardInsert(BoardVO vo, MultipartFile uploadFile) throws IllegalStateException, IOException {			
		...
		if(!uploadFile.isEmpty() && uploadFile.getSize() > 0) {			
			...
			System.out.println("업로드 파일정도"+filename+" : "+filesize);
		}
